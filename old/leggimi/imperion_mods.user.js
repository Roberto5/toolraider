...
Imperion = {
	...
	getReportByUrl:function(url) {
	    race=0;
        if (Imperion.getRace()=='terrans') { race = 1;  }
        if (Imperion.getRace()=='titans') { race = 2;  }
		if (Imperion.getRace()=='xen') { race = 3;  }
		...
		switch(report.type) {
			case 'combat':
				...
				try {
				    var myFleet = $('div.content table:eq(0)',dom)[0]
                      var capacity = 0;
                      for (var i = 0; i < 12; i++) {
                            if (parseInt(myFleet.rows[2].cells[i+1].innerHTML) > 0)
                            {capacity += myFleet.rows[2].cells[i+1].innerHTML.replace(".","") * CalcShipCapacity(shipIDs[race-1][i]);}
                      }
					...
					report.resourcesGathered = {
                        ...
                        stor : parseInt(capacity)					
					};
				} catch(e) {
				    
					report.resourcesGathered = {metal:0, crystal:0, fuel:0, qi:0, stor:0};
				}
				report.parsed = true;
				break;
			case 'recycle':
                ...
                try {
                    var myFleet = $('div.content table',dom)[0];
                    capacity=0;
				    switch(race)
                    {
                        case 1:
                       //terani
                       capacity = myFleet.rows[2].cells[10].innerHTML.replace(".","") * CalcShipCapacity(shipIDs[race-1][9]);
                       capacity += myFleet.rows[2].cells[11].innerHTML.replace(".","") * CalcShipCapacity(shipIDs[race-1][10]);
                        break;
                        case 2:
                       //titani
                       capacity = myFleet.rows[2].cells[11].innerHTML.replace(".","") * CalcShipCapacity(shipIDs[race-1][10]);
					...
					report.resourcesGathered = {metal:0, crystal:0, fuel:0 , stor :0};
				}
				report.parsed = true;
				break;
			case 'espionage':
					...
					if(reportLink.size() == 1) {
                        ...
                        switch(report.type) {
							case 'combat':
                                ...
                                tot = (report.resourcesGathered.metal > 0 ? parseInt(report.resourcesGathered.metal) : 0) + (report.resourcesGathered.crystal > 0 ? parseInt(report.resourcesGathered.crystal) : 0) + (report.resourcesGathered.fuel > 0 ? parseInt(report.resourcesGathered.fuel) : 0);
								reportLink.after(
									...
									(report.resourcesGathered.fuel > 0 ? ' <img src="/img/interface/icon/deuterium.png" title="Fuel" class="resourceIconReport"/>' + addCommas(report.resourcesGathered.fuel) : '') +
									(report.resourcesGathered.qi > 0 ? ' <img src="' + icons.qi + '" class="resourceIconReport"/>' + addCommas(report.resourcesGathered.qi) : '') 
                                    + ' = '+  tot +" "+  ( tot>0 ? Math.round(100 * tot / report.resourcesGathered.stor) + "%" : '')
								);
								break;								
							case 'espionage':
							...
							case 'recycle':
								...
								reportLink.after(
								    ...
								    + ' = '+  tot +" "+ Math.round(100 * tot / report.resourcesGathered.stor) + "%"
                                );
